    ...
    Microsoft::WRL::ComPtr<ID3D12Resource>& uploadBuffer)
{
    ...
    CD3DX12_RESOURCE_DESC aBuffer = CD3DX12_RESOURCE_DESC::Buffer(byteSize);
    CD3DX12_HEAP_PROPERTIES type_defalut = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    ...
    CD3DX12_RESOURCE_BARRIER common_copy_dest = CD3DX12_RESOURCE_BARRIER::Transition(defaultBuffer.Get(),
        D3D12_RESOURCE_STATE_COMMON,
        D3D12_RESOURCE_STATE_COPY_DEST); 

    CD3DX12_RESOURCE_BARRIER copyDest_GenericRead = CD3DX12_RESOURCE_BARRIER::Transition(defaultBuffer.Get(),
        D3D12_RESOURCE_STATE_COPY_DEST,
        D3D12_RESOURCE_STATE_GENERIC_READ);

    cmdList->ResourceBarrier(1, &common_copy_dest);
    UpdateSubresources<1>(cmdList, defaultBuffer.Get(), uploadBuffer.Get(), 0, 0, 1, &subResourceData);
    cmdList->ResourceBarrier(1, &copyDest_GenericRead);
